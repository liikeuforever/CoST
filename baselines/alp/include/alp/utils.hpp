...
struct AlpApiUtils {
	...
	static void fill_incomplete_alp_vector(double*   input_vector,
		...
		encode(input_vector, exceptions, exceptions_positions, exceptions_count, encoded_integers, stt);
	...
	static void fill_incomplete_alprd_vector(double* input_vector, const state& stt) {
		// We just fill the vector with the first value
		const double first_vector_value = input_vector[0];
		for (size_t i = stt.vector_size; i < config::VECTOR_SIZE; i++) {
